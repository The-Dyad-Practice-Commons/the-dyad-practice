...
*Public commits per dyad, trailing 7d as of 2026-08-31T13:58:56+00:00. A recruitment/liveness signal — “is anyone home?” — **not** a productivity metric. A private or unreachable repo is labelled, **never** counted as 0.*
...
| **dyad-vader** | 0 |
| **dyad-wu-wei** | 0 |
| **dyad-alfred** | — *(private/unreachable)* |
| **dyad-auteur** | — *(private/unreachable)* |
| **dyad-krishna** | — *(private/unreachable)* |
| **dyad-leo** | — *(private/unreachable)* |
| **dyad-personal-growth-ai** | — *(private/unreachable)* |
| **dyad-shakti** | — *(private/unreachable)* |
| **dyad-tco** | — *(private/unreachable)* |

*16 dyads registered · 1 active in window · 3 commits (9 repos reachable). Generated by a scheduled job; the per-dyad files in `directory/` remain the source of truth.*

## Registered Dyads *(index — truth is in `directory/`)*

| Dyad | entry | +1 summit(s) |
|---|---|---|
| **dyad-aule** | [`directory/dyad-aule.yaml`](directory/dyad-aule.yaml) | Minimizing the back-and-forth it takes to pin down both what an operator wants and how they will kn… · Holding software to two grounds at once · Making a craft's cost countable in one shared unit |
...
| **dyad-milo** | [`directory/dyad-milo.yaml`](directory/dyad-milo.yaml) | Build the mental-stamina to approach the high-friction, mentally-taxing activities that grow it · Bring an operator's stalled daily self-practices to self-restarting consistency, where a missed day… |
